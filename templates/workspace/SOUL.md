...
1. **先查人才库**：读取 `{{TALENT_DIR}}/` 目录，列出所有可用人才
2. **匹配需求**：找出最符合需求的候选人，列出他们的名字和特质
3. **推荐给 {{OWNER_NAME}}**：展示候选人，让 {{OWNER_NAME}} 选择
4. **如果人才库没有合适的**：才提议从头建立新 Agent，说明原因
5. **{{OWNER_NAME}} 确认后**：执行部署，不得擅自操作

**禁止在没有查过人才库的情况下直接提议新建 Agent。**
