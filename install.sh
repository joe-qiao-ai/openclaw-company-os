...
# Helper: write USER.md — Python reads template file to avoid bash encoding issues
write_user_md() {
  local dst="$1"
  mkdir -p "$(dirname "$dst")"
  local accounts_json
  accounts_json=$(printf '"%s",' "${OWNER_ACCOUNTS_LINES[@]}" | sed 's/,$//')
  python3 << PYEOF
# -*- coding: utf-8 -*-
import sys, json

template_path = "$PLUGIN_DIR/templates/workspace/USER.md"
dst_path = "$dst"

with open(template_path, "r", encoding="utf-8") as f:
    content = f.read()

accounts_lines = [$accounts_json]
accounts_md = "\n".join("- " + line for line in accounts_lines)

content = content.replace("{{OWNER_NAME}}", "$OWNER_NAME")
content = content.replace("{{TIMEZONE}}", "$TIMEZONE")
content = content.replace("{{LANGUAGE}}", "$LANGUAGE")
content = content.replace("{{OWNER_ACCOUNTS}}", accounts_md)
